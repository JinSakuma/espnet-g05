...
case "$csjv" in
    ...
    "usb" ) SDB=MORPH/SDB/ ; WAV=WAV/ ; disc="core" ;; # Set SDB directory and WAV directory respectively.
    #"usb" ) SDB=MORPH/SDB/ ; WAV=WAV/ ; disc="core noncore" ;; # Set SDB directory and WAV directory respectively.
    #"usb" ) SDB=MORPH/SDB/ ; WAV=WAV/ ; disc="core ds TS_A TS_B" ;; # Set SDB directory and WAV directory respectively.
    ...
    for vol in $disc ;do
        ...
        (
            if [ $csjv = "merl" ]; then
                ...
                #ids=`ls /mnt/aoni01/db/CSJ/CSJ2004/VOL18/CU/$vol | sed 's:_CU.xml::g' | sed 's/00README.txt//g'`
            fi
            ...
            for id in $ids; do
                ...
                #python3 local/csj_make_trans/csj2clause.py \
                #    --input $TPATH/${id} \
                #    --output1 $outd/$vol/$id/${id}-trans.text \
                #    --output2 $outd/$vol/$id/${id}.4lex
...
(
    ...
    mv $outd/eval/{A01M0110,A01M0137,A01M0097} $outd/eval/eval1
    mv $outd/eval/{A01M0056,A03F0072} $outd/eval/eval2
    mv $outd/eval/{S00M0112,S00F0066,S00M0213} $outd/eval/eval3
    ...
    [ 3 -eq `ls $outd/eval/eval1 | wc -l` ] && echo -n >$outd/eval/.done_eval1
    [ 2 -eq `ls $outd/eval/eval2 | wc -l` ] && echo -n >$outd/eval/.done_eval2
    [ 3 -eq `ls $outd/eval/eval3 | wc -l` ] && echo -n >$outd/eval/.done_eval3
